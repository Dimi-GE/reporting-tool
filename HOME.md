...
**Expand** pulls the full Expenses by Category chart — the same component formerly on the Dashboard (see DASHBOARD.md) — out into a floating window over the page, following the identical principle as Recent Transactions' **Full History** overlay: it grows out from its card to fill the same `.content-container`-sized, vertically-centered footprint, with a dimmed backdrop behind it that dismisses it on click. Inside: a bar chart across all 12 expense categories for a selected date range, sorted descending by spend, with a toggleable legend and period-scoped export/import. It defaults to the current calendar month, independent of the donut's fixed month scope. **Collapse** (or the backdrop) shrinks it back down. It stays open across an in-Home refresh (e.g. after editing an entry) but always starts closed on a fresh visit to Home.
